...
<div id="page" class="clearfix container_9">
	...
	<div id='categoryTopHook'>{$HOOK_TOP_CATEGORY}</div>
	...
	<div id="pb-left-column" class='mceContentBody'>
        ...
        {if !((!$allow_oosp && $product->quantity <= 0) OR !$product->available_for_order OR (isset($restricted_country_mode) AND $restricted_country_mode) OR $PS_CATALOG_MODE)}
            <p>
                {if $customer->getActiveWebSubscription() !== null && $product->is_virtual}
                    {assign "free" true}
                    <a class="button_mini color-myaccount" href="{$link->getPageLink('get-file', true, NULL, "pkey={$hash|escape:'htmlall':'UTF-8'}")}" onclick="ga('send', 'pageview', [location.pathname + '/detail/direct-download/' + {$product->id}]);ga('send', 'event', 'a', 'b', 'c', 'd');">Télécharger (votre abonnement vous donne accès)</a>
		...
		<div id="short_description_block">
			{if $product->description_short}
				<div id="short_description_content" class="rte mceContentBody align_justify">{$product->description_short}</div>
			{/if}
						...
						<fieldset class="attribute_fieldset">
							...
							{if ($group.group_type == 'select')}
								<select name="{$groupName}" id="group_{$id_attribute_group|intval}" class="attribute_select" onchange="findCombination();getProductAttribute();">
									{foreach from=$group.attributes key=id_attribute item=group_attribute}
										{if $id_attribute_group != 4 || $id_attribute != 24}
							...
							{elseif ($group.group_type == 'radio')}
								<ul class='radio'>
									{foreach from=$group.attributes key=id_attribute item=group_attribute}
										<li>
											<input id="{$id_attribute}" type="radio" class="attribute_radio" name="{$groupName}" value="{$id_attribute}" {if ($group.default == $id_attribute)} checked="checked"{/if} onclick="findCombination();getProductAttribute();" />
...
<div id="more_info_block" class="clear">
	...
	{if isset($features) && $features}
		...
		{foreach from=$features item=feature}
            {if isset($feature.value)}
			    <li><span>{$feature.name|escape:'htmlall':'UTF-8'} : {$feature.value|escape:'htmlall':'UTF-8'}</span></li>
            {/if}
		{/foreach}
		</ul>
	{/if}
	...
	{if isset($accessories) AND $accessories}
		...
		{foreach from=$accessories item=accessory name=accessories_list}
			{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND !isset($restricted_country_mode)} {*AND $accessory.available_for_order*}
				{assign var='accessoryLink' value=$link->getProductLink($accessory.id_product, $accessory.link_rewrite, $accessory.category)}
				<li class="ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
						<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}">N° {$accessory.reference|regex_replace:"/[_-]/":" p."} : {$accessory.name|escape:'htmlall':'UTF-8'}</a>
				</li>
			{/if}
		{/foreach}
		</ul>
	...
	{if isset($product) && $product->customizable}
		<div id="idTab10" class="bullet customization_block">
			<form method="post" action="{$customizationFormTarget}" enctype="multipart/form-data" id="customizationForm" class="clearfix mceContentBody">
				<p class="infoCustomizable">
					{l s='Saisissez au moins une adresse e-mail pour autoriser son accès à votre abonnement. N\'oubliez pas d\'enregistrer  et ensuite d\'ajouter au panier.'}
				...
				<div class="customizableProductsText">
					...
					{foreach from=$customizationFields item='field' name='customizationFields'}
						...
						<li class="customizationUploadLine{if $field.required} required{/if}">
							...
							<input type="text" name="textField{$field.id_customization_field}" id="textField{$customizationField}" rows="1" cols="40" class="customization_block_input" value='{if isset($textFields.$key)}{$textFields.$key|stripslashes}{/if}' />
						</li>
						{counter}
						{/if}
					{/foreach}
