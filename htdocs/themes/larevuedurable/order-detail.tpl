...
<p>
	<a class='button_large' target="_blank" href="{$link->getPageLink('pdf-invoice', true)}?id_order={$order->id|intval}{if $is_guest}&secure_key={$order->secure_key}{/if}">{l s='Download your invoice as a PDF file.'}</a>
</p>
{/if}
{if $order->recyclable}
	...
	<table class="std">
		<thead>
			<tr>
				...
				{/if}
				<th class="item price_cell" style='width:80px'>{l s='Unit price'}</th>
				<th class="last_item price_cell" style='width:80px'>{l s='Total price'}</th>
			</tr>
		</thead>
		<tfoot>
			{if $priceDisplay && $use_tax}
				<tr class="item price_cell">
			...
			<tr class="totalprice price_cell item">
				<td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
						...
						<td class="bold">
							<label for="cb_{$product.id_order_detail|intval}">
								{if $product.download_hash && $invoice && $product.display_filename != '' && $product.product_quantity_refunded == 0 && $product.product_quantity_return == 0}
									...
									<span class='virtual_file_label'>{$product.product_name|escape:'htmlall':'UTF-8'} </span>
									
									{if isset($is_guest) && $is_guest}
										<a class='button_large' href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}&id_order={$order->id}&secure_key={$order->secure_key}")}" title="{l s='Download this product'}">
									{else}
										<a class='button_large' href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}")}" title="{l s='Download this product'}">
									{/if}
									
									{l s='Télécharger fichier joint'}
									</a>
								{else}
