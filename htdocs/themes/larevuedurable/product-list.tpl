	...
	{foreach from=$products item=product name=products}
		<li class="ajax_block_product {if $smarty.foreach.products.first}first_item{elseif $smarty.foreach.products.last}last_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}item{/if} clearfix">
			...
			<div class="center_block mceContentBody">
				...
				<h2><a href="{$product.link|escape:'htmlall':'UTF-8'}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|escape:'htmlall':'UTF-8'}</a></h2>
			...
			<div class="right_block">
				...
				<div class='product_list_buttons'>
                    ...
                    {if isset($product.tags)}
                        {foreach from=$product.tags item=tag name=tag}
                            <div class='tag tag{$tag.id}'>{$tag.name}</div>
                        {/foreach}
                    {/if}
					...
					{if isset($category) && $category->id != 19}
						...
						{/if}
					{/if}
					<a class="button_large lnk_view" href="{$product.link|escape:'htmlall':'UTF-8'}" title="{l s='View'}">{l s='Voir détail'}</a>
				</div>
			</div>
		</li>
	{/foreach}
