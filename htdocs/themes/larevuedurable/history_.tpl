...
<div class='mceContentBody'><p>{l s='Here are the orders you\'ve placed since your account was created.'}</p></div>
	...
	<table id="order-list" class="std">
		<thead>
			<tr>
				...
				<th class="item" style="width:75px">{l s='Date'}</th>
				<th class="item" style="width:75px;text-align:right;">{l s='Total price'}</th>
		...
		{foreach from=$orders item=order name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				...
				<td class="history_price"  style="text-align:right;"><span class="price">{displayPrice price=$order.total_paid currency=$order.id_currency no_utf8=false convert=false}</span></td>
				...
				{if (isset($order.invoice) && $order.invoice && isset($order.invoice_number) && $order.invoice_number) && isset($invoiceAllowed) && $invoiceAllowed == true}
					<a class='button_mini' href="{$link->getPageLink('pdf-invoice', true, NULL, "id_order={$order.id_order}")}" title="{l s='Invoice'}" class="_blank">{l s='PDF'}</a>
				{else}-{/if}
				</td>
				<td class="history_detail">
					<a class="button_mini color-myaccount" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)}');">{l s='details'}</a>
					<!--
					{if isset($opc) && $opc}
					<a class="button_mini" href="{$link->getPageLink('order-opc', true, NULL, "submitReorder&id_order={$order.id_order}")}" title="{l s='Reorder'}">
					{else}
					<a class="button_mini" href="{$link->getPageLink('order', true, NULL, "submitReorder&id_order={$order.id_order}")}" title="{l s='Reorder'}">
					{/if}
						{l s='Reorder'}
					</a>
					-->
