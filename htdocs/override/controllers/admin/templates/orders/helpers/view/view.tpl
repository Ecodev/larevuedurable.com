...
<div class="bloc-command">
	...
	<div class="container-command">
		...
		<div style="width: 49%; float:right;">
			...
			<!-- Payments block -->
			<fieldset {if $order->getOrderPaymentCollection()} class='noprint' {/if}>
	...
	<form style="width: 98%" class="container-command-top-spacing" action="{$current_index}&vieworder&token={$smarty.get.token}&id_order={$order->id}" method="post" onsubmit="return orderDeleteProduct('{l s='This product cannot be returned.'}', '{l s='Quantity to cancel is greater than quantity available.'}');">
		...
		<fieldset style="width: 100%; ">
			...
			<div style="float:left;width: 100%;">
				{if $can_edit}
				{if !$order->hasBeenDelivered()}<div class='noprint' style="float: left;"><a href="#" class="add_product button"><img src="../img/admin/add.gif" alt="{l s='Add a product'}" /> {l s='Add a product'}</a></div>{/if}
				<div class='noprint' style="float: right; margin-right: 10px" id="refundForm">
				<!--
				...
				<br class='noprint' clear="left" /><br />
				{/if}
				...
				<div style="float:right; margin-top: 20px;">
					<table class="table" width="450px;" style="border-radius:0px;"cellspacing="0" cellpadding="0">
						...
						<tr style="font-size: 14px" id="total_order">
							<td style="font-size: 14px">{l s='Total'}</td>
							<td class="amount" style="font-size: 14px" align="right">
				...
				<div class='noprint' style="float:right; width:450px; margin-top:15px;">
			...
			<div class='noprint'  style="clear:both; height:15px;">&nbsp;</div>
	...
	<div {if !(sizeof($messages))}class='noprint'{/if} style="width: 49%; float: left">
	...
	{if (sizeof($messages))}
		...
		{foreach from=$messages item=message}
			<div style="overflow:auto;" {if $message['is_new_for_me']}class="new_message"{/if}>
	...
	<div class='noprint'style="width: 49%; float: right">
		<form action="{$smarty.server.REQUEST_URI}&token={$smarty.get.token}" method="post" >
            <fieldset >
                ...
                <div>
                    ...
                    <input type='text' name='newOrderDate' placeholder='{$order->date_add}'/>
                    <input type="hidden" name="id_order" value="{$order->id}" />
                    <input type="hidden" name="id_customer" value="{$order->id_customer}" />
                    <input type="submit" class="button" name="submitDate" value="{l s='Send'}" />
                </div>
            </fieldset>
		</form>
	</div>
	...
	<br class='noprint'  /><br class='noprint' /><a class='noprint' href="{$current_index}&token={$smarty.get.token}"><img src="../img/admin/arrow2.gif" /> {l s='Back to list'}</a><br />
{/block}
