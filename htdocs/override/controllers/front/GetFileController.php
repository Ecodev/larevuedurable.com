...
class GetFileController extends GetFileControllerCore
{
	...
	protected function applyWatermark($customer, $filename, $file)
	{
		...
		$pdf = new TCPDF('P', 'mm', array('width'=>23, 'height'=>31), true, 'UTF-8', false);
		// disable header and footer
		$pdf->setPrintHeader(false);
		$pdf->setPrintFooter(false);
		$pdf->SetFont('HelveticaI', '', 14);
		$pdf->AddPage();
		$pdf->Write(0, $text , '', false, 'R', true);
		$watermark = 'download_tmp/watermark-'.$customer->id.'.pdf';
		$pdf->Output(_PS_ROOT_DIR_.'/'.$watermark, 'F');

		// Merge the watermark with the input PDF
		$ps_root_dir = str_replace(' ', '\ ', _PS_ROOT_DIR_);

		exec( PDFTK_PATH.' '.str_replace(' ', '\ ', $file).' stamp '.$ps_root_dir.'/'.$watermark.' output '.$ps_root_dir.'/download_tmp/'.$customer->id.'-'.$filename, $output , $return);
		unlink(_PS_ROOT_DIR_.'/'.$watermark);
