...
class GetFileController extends GetFileControllerCore
{
    ...
    public function init()
    {
        ...
        /**
         * If the key is valid (corresponds to a filename on server) and if the product is virtual and customer has active subscription with archive access, allow download.
        ...
        $mimeType = $this->detectMimeType($file, $filename);

        $finfo = finfo_open(FILEINFO_MIME_TYPE);

        if (finfo_file($finfo, $file) == 'application/pdf')
        {
            $file = $this->applyWatermark( $this->context->customer, $filename, $file );
        }
    ...
    /**
     * Customise a PDF virtual product by stamping every page with customer info
     *
     * @param Customer $customer the customer who downloads
     * @param string $filename name of the downloaded file
     * @param string $file local path to the PDF file
     * @return string local path to the stamped PDF
     */
    protected function applyWatermark($customer, $filename, $file)
    {

        if( file_exists(_PS_ROOT_DIR_.'/download_tmp/'.$customer->id.'-'.$filename) )
            return _PS_ROOT_DIR_.'/download_tmp/'.$customer->id.'-'.$filename;

        $text =  $customer->firstname .' '.$customer->lastname .', '.$customer->email;

        // Compose a PDF with the watermark stamp
        ...
        $file = _PS_ROOT_DIR_ . '/download_tmp/' . $customer->id . '-' . $filename;
        $pdf->Output($file, 'F');
        return $file;
    }
