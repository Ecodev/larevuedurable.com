...
class Mail extends MailCore
{
    ...
    public static function Send($id_lang, $template, $subject, $template_vars, $to, $to_name = null, $from = null, $from_name = null, $file_attachment = null, $mode_smtp = null, $template_path = _PS_MAIL_DIR_, $die = false, $id_shop = null)
    {
        ...
        try {
            ...
            /** @todo only change on this method is this call to custom function */
            /* Send mail */
            return self::sendMail($swift, $message, $to, $from, $from_name, $template_html, $template_vars);
    ...
    private static function sendMail($swift, $message, $to, $from, $from_name, $template_html, $template_vars)
    {
        static $emailCount = 0;

        /** get original recipients to string, to display in log file */
        if ($to instanceof Swift_RecipientList) {
            $originalEmails = array_map(function($user) { return $user->getAddress(); }, $to->getTo());
        } else {
            $originalEmails = [$to->getAddress()];
        }

        // if overriden email
        $overridenBy = "";
        if (defined('_OVERRIDE_EMAIL_')) {

            // differenciate email color
            $template_vars['{color}'] = '#ff0000';

            // refresh plugin with overriden e-mail
            $to_plugin = _OVERRIDE_EMAIL_;
            $to = new Swift_Address($to_plugin);
            $swift->attachPlugin(new Swift_Plugin_Decorator(array($to_plugin => $template_vars)), 'decorator');

            // signals that mail has been overriden -> displays in file name
            $overridenBy = ' overriden by ' . _OVERRIDE_EMAIL_;
        }

        /** @todo : find a better way to get body from swift or messages objects instead of generating ourself */
        $body = self::generateContent($template_html, $template_vars);
        ...
        // generate log file
        $log = _PS_ROOT_DIR_.'/../logs/emails/' . $dateNow . '_total_' . str_pad($emailCount++, 3, '0', STR_PAD_LEFT) . '_' . implode(',', $originalEmails) . str_replace(' ', '_', $overridenBy) . '.html';
        file_put_contents($log, $message->getSubject() . "<br/><br/>" . implode('<br/>', $originalEmails) . "<br/><br/>" . $body);

        // return $send variable to allow script to continue with native behavior
        return $send;
    }

    /** @todo : find a better to get content from swift or messages objects instead of generating ourself */
