...
class BankWireBVR extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Virement bancaire avec BVR');
	...
	public function install()
	{
        $os = new OrderState();
        $os->name = array(
            1 => 'En attente de paiement par BVR',
            2 => 'En attente de paiement par BVR'
        );
        $os->send_email = true;
        $os->template = 'bankwirebvr';
        $os->module_name = 'bankwirebvr';
        $os->color = '#ffadae';
        $createdOS = $os->add();
        $movedLogo = copy(__dir__.'/logo.gif', _PS_OS_IMG_DIR_.$os->id.'.gif');

		if (!parent::install() ||  !$createdOS || !$movedLogo || !$this->registerHook('payment') || !$this->registerHook('paymentReturn') || !Configuration::updateValue('PS_OS_BANKWIRE_BVR', $os->id))
			return false;
		return true;
	}
	...
	{
        unlink( _PS_OS_IMG_DIR_.Configuration::get('PS_OS_BANKWIRE_BVR').'.gif');
        $os = new OrderState(Configuration::get('PS_OS_BANKWIRE_BVR'));
        $os->delete();

		if (!Configuration::deleteByName('PS_OS_BANKWIRE_BVR') ||
			!parent::uninstall())
			return false;
		return true;
	}
	...
	public function hookPaymentReturn($params)
	{
		...
		if ($state == Configuration::get('PS_OS_BANKWIRE_BVR') || $state == Configuration::get('PS_OS_OUTOFSTOCK'))
		{
			$this->smarty->assign(array(
