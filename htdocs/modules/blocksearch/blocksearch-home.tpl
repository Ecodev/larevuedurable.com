...
<div id="search_block_home">
	...
	<label id='search_label' for="search_query_top">{l s='Accéder à nos infos | dossiers | articles' mod='blocksearch'}</label>
	...
	<form method="get" action="{$link->getPageLink('search')}" id="searchbox">
		<p>
			...
			<input type="hidden" name="orderby" value="position" />
			<input type="hidden" name="orderway" value="desc" />
			<input placeholder='{l s='rechercher...' mod='blocksearch'}' class="search_query" type="text" id="search_query_home" name="search_query" value="{if isset($smarty.get.search_query)}{$smarty.get.search_query|htmlentities:$ENT_QUOTES:'utf-8'|stripslashes}{/if}" />
