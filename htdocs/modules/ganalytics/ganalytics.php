...
{	
	...
	function uninstall()
	{
		if (!Configuration::deleteByName('GANALYTICS_ID') || !Configuration::deleteByName('GANALYTICS_DOMAIN') || !parent::uninstall())
			return false;
		return true;
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitGAnalytics') AND ($gai = Tools::getValue('ganalytics_id') AND $gad = Tools::getValue('ganalytics_domain')))
		{
			Configuration::updateValue('GANALYTICS_ID', $gai);
			Configuration::updateValue('GANALYTICS_DOMAIN', $gad);
			$output .= '
	...
	public function displayForm()
	{
		...
		<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
			<fieldset class="width2">
				...
				<label>'.$this->l('Votre nom de domaine').'</label>
                <div class="margin-form">
					<input type="text" name="ganalytics_domain" value="'.Tools::safeOutput(Tools::getValue('ganalytics_domain', Configuration::get('GANALYTICS_DOMAIN'))).'" />
					<p class="clear">'.$this->l('Example:').' ecodev.ch</p>
				</div>
				<center><input type="submit" name="submitGAnalytics" value="'.$this->l('Enregistrer').'" class="button" /></center>
			</fieldset>
		</form>';
	...
	function hookHeader($params)
	{
		...
		$this->context->smarty->assign('ganalytics_domain', Configuration::get('GANALYTICS_DOMAIN'));
