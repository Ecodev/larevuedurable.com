...
class editorial3 extends Module
{
	
	private $default_hook = 'displayHomeCol3';
		
	public function __construct()
	{
		$this->name = 'editorial3';
		$this->tab = 'home';
		$this->version = '2.0';
		...
		$this->displayName = $this->l('Editeur de page d\'accueil (zone de droite). 2e bloc');
		$this->description = $this->l('A text-edit module for your homepage.');
		$path = dirname(__FILE__);
		if (strpos(__FILE__, 'Module.php') !== false)
			$path .= '/../modules/'.$this->name;
		include_once $path.'/EditorialClass3.php';
	}

	public function install()
	{
		
		Tools::createHookIfNoExist($this->default_hook, 'Accueil : troisième colonne');
		
		if (!parent::install() || !$this->registerHook($this->default_hook) || !$this->registerHook('displayHeader'))
			return false;

		$res = Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'editorial3` (
		...
		if ($res)
			$res &= Db::getInstance()->execute('
				CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'editorial3_lang` (
	...
	{
		$res = Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'editorial3`');
		$res &= Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'editorial3_lang`');
	...
	public function getContent()
	{
		...
		$editorial = EditorialClass3::getByIdShop($id_shop);
	...
	public function hookDisplayHomeCol3($params)
	{
		if (!$this->isCached('editorial3.tpl', $this->getCacheId()))
		{
			$id_shop = (int)$this->context->shop->id;
			$editorial = EditorialClass3::getByIdShop($id_shop);
			if (!$editorial)
				return;			
			$editorial = new EditorialClass3((int)$editorial->id, $this->context->language->id);
			if (!$editorial)
				return;
		...
		return $this->display(__FILE__, 'editorial3.tpl', $this->getCacheId());
	}
