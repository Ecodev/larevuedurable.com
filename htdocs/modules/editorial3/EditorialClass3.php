...
class EditorialClass3 extends ObjectModel
{
	...
	{
		$id = Db::getInstance()->getValue('SELECT `id_editorial` FROM `'._DB_PREFIX_.'editorial3` WHERE `id_shop` ='.(int)$id_shop);
		if ($id)
			return new EditorialClass3((int)$id);
		else
			return false;
	}
