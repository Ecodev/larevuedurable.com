...
class Editorial2 extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Editeur de page d\'accueil (zone de droite). 1er bloc');
		$this->description = $this->l('A text-edit module for your homepage.');
		$path = dirname(__FILE__);
		if (strpos(__FILE__, 'Module.php') !== false)
			$path .= '/../modules/'.$this->name;
		include_once $path.'/EditorialClass2.php';
	}
	...
	public function install()
	{
		...
		$res = Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'editorial2` (
		...
		if ($res)
			$res &= Db::getInstance()->execute('
				CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'editorial2_lang` (
	...
	{
		$res = Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'editorial2`');
		$res &= Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'editorial2_lang`');
	...
	public function getContent()
	{
		...
		$editorial = EditorialClass2::getByIdShop($id_shop);
	...
	{
		if (!$this->isCached('editorial2.tpl', $this->getCacheId()))
		{
			$id_shop = (int)$this->context->shop->id;
			$editorial = EditorialClass2::getByIdShop($id_shop);
			if (!$editorial)
				return;			
			$editorial = new EditorialClass2((int)$editorial->id, $this->context->language->id);
			if (!$editorial)
				return;
		...
		return $this->display(__FILE__, 'editorial2.tpl', $this->getCacheId());
	}
