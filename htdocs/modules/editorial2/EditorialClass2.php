...
class EditorialClass2 extends ObjectModel
{
	...
	{
		$id = Db::getInstance()->getValue('SELECT `id_editorial` FROM `'._DB_PREFIX_.'editorial2` WHERE `id_shop` ='.(int)$id_shop);
		if ($id)
			return new EditorialClass2((int)$id);
		else
			return false;
	}
