...
/*		var $proxy = "proxy.prestashop.miami";
		var $proxyPort = 3128;
		var $proxyAuth = "mbertholino:the man";
*/
		var $proxy = null;
		var $proxyPort = 80;
		var $proxyAuth = null;
    ...
    */
    ...
    function callServer($method, $params) {
        ...
        $postUrl = $this->proxy ? "http://$host/".$this->apiUrl["path"] : $this->apiUrl['path'];

        $payload = "POST " . $postUrl . "?" . $this->apiUrl["query"] . "&method=" . $method . " HTTP/1.0\r\n";
				if ($this->proxy && $this->proxyAuth) {
					$payload .= "Proxy-Authorization: Basic ".base64_encode($this->proxyAuth)."\r\n";
				}
        $payload .= "Host: " . $host . "\r\n";
        ...
        ob_start();
				...
				if ($this->proxy)
				{
					$sock = fsockopen($this->proxy, $this->proxyPort, $errno, $errstr, 30);
				}
        else if ($this->secure){
