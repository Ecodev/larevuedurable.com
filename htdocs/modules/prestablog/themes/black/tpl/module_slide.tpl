...
<div id="prestablog_slide_home">
	...
	<div class="prestablog_visuels">
		...
		{if $slide.title!=''}
			<h2>{if isset($slide.link_for_unique)}<a href="{PrestaBlogUrl id=$slide.id_prestablog_news seo=$slide.link_rewrite titre=$slide.title}" title="{$slide.title}">{/if}{$slide.title}{if isset($slide.link_for_unique)}</a>{/if}</h2>
		{/if}
