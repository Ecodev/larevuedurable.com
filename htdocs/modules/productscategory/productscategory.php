...
class productsCategory extends Module
{
	...
	public function install()
	{
		...
		// 'HOOK_PRODUCT_TAB_CONTENT' =>  displayProductTabContent
	 	...
	 	if (!parent::install() OR !$this->registerHook('displayProductTabContent') OR !$this->registerHook('displayProductTab') OR  !$this->registerHook('header') OR !Configuration::updateValue('PRODUCTSCATEGORY_DISPLAY_PRICE', 0))
	 		return false;
	 	return true;
	}
	...
	public function hookDisplayProductTab($params)
	{
		...
		// Get infos
		$categoryProducts = $category->getProducts($this->context->language->id, 1, 10); /* 100 products max. */
		
//		$finalProducts = array();
//		foreach($categoryProducts as $product)
//		{
//			echo $product['reference'].'-';
//			if( preg_match("/^[0-9][0-9][0-9]$/", $product['reference']) ) array_push($finalProducts,$product);
//			else echo 'false';
//			echo "\n";
//		}
