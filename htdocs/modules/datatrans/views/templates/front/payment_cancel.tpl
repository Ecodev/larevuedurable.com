...
<div id="page" class="clearfix container_9">
    ...
    <div id="columns" class="grid_9 alpha omega clearfix mceContentBody">
        ...
        <h1 style="margin-top: 40px; line-height: 1.4em; color:red;">{$shop_name} - {$title} : {l s='transaction canceled' mod='datatrans'}</h1>
        <p>{l s='You have canceled the payment transaction.' mod='datatrans'}</p>
        <p><a href="{$link->getPageLink('order', true, NULL, NULL)}" class="button_large">&laquo; {l s='Go back your Cart' mod='datatrans'}</a></p>